...
def _arakawa_stencil_2d(
    ...
    """
    Internal: Full Arakawa (1966) J++ scheme on a single 2D slice.
    
    Implements energy + enstrophy conserving 9-point stencil.
    ...
    J : np.ndarray, shape (nr, nθ)
        Jacobian [f, g] using full Arakawa J++ scheme
    
    Notes
    -----
    **Full Arakawa Scheme (Arakawa 1966, Eq. 2.16):**
    ...
    Three formulations:
    - J₁: Simple centered differences
    - J₂: Upper diagonal (enstrophy conserving)
    - J₃: Lower diagonal (stability)
    
    Average: J = (J₁ + J₂ + J₃) / (12·dr·dθ)
    
    **Conservation Properties:**
    - Energy: ∫ ψ [ψ, ω] = 0 (to machine precision)
    - Enstrophy: ∫ ω [ψ, ω] = 0 (to machine precision)
    
    **Implementation:**
    Adapted from pytokeq/core/operators.py::compute_poisson_bracket_arakawa()
    Modified for cylindrical (r,θ) coordinates with metric factor 1/R²
    ...
    **References:**
    ...
    for i in range(1, nr-1):
        for j in range(ntheta):
            ...
            # --- Combine with Arakawa averaging ---
            J_arakawa = (J1 + J2 + J3) / (12.0 * dr * dtheta)
            
            # --- Apply metric factor: 1/R² ---
            J[i, j] = J_arakawa / R[i, j]**2
    
    # Boundary handling (radial)
    # Set to zero at r=0 and r=a (Dirichlet BC enforced elsewhere)
    J[0, :] = 0.0
    J[-1, :] = 0.0
